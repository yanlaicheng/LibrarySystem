...
import com.yan.LibrarySystem.service.BookService;
import com.yan.LibrarySystem.service.BookTypeService;
import com.yan.LibrarySystem.utils.ExportExcelUtil;
...
public class BookController extends BaseController {
	...
	public String add(Model model, HttpServletRequest request) {
		...
		try {
			bookTypeList = bookTypeService.QueryAllBookTypeInfo();
			request.setAttribute("bookTypeList", bookTypeList);
			return "Book_add";
		} catch (Exception e) {
			e.printStackTrace();
			request.setAttribute("error", "add[GET] error!");
			return "error";
		}
	}

	@RequestMapping(value = "/add", method = RequestMethod.POST)
	public String add(@Validated Book book, BindingResult br, Model model, HttpServletRequest request) {
		try {
			if (br.hasErrors()) {
			...
			if (bookService.getBookByBarcode(book.getBarcode()) != null) {
				throw new UserException(URLEncoder.encode("图书条形码已经存在!","utf-8"));
			}
			book.setPhotoBook(this.handlePhotoFileUpload(request, "photoBookFile"));
			bookService.addBook(book);
			request.setAttribute("message", URLEncoder.encode("图书添加成功!", "utf-8"));
			return "message";
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String list(String barcode, @ModelAttribute BookType bookType, String bookName, String publishDate,
	...
	public String frontlist(String barcode, @ModelAttribute BookType bookType, String bookName, String publishDate,
			Integer currentPage, Model model, HttpServletRequest request) {
		try {
			if (currentPage == null || currentPage == 0)
				currentPage = 1;
	...
	public String update(@Validated Book book, BindingResult br, Model model, HttpServletRequest request) {
		try {
			if (br.hasErrors()) {
				model.addAttribute(book);
				/* 查询所有的BookType信息 */
			...
			if (!photoBookFileName.equals("upload/NoImage.jpg"))
				book.setPhotoBook(photoBookFileName);

			BookType bookType = bookTypeService.GetBookTypeByBookTypeId(book.getBookType().getBookTypeId());
			book.setBookType(bookType);
			bookService.updateBook(book);
			request.setAttribute("message", URLEncoder.encode("图书更新成功!", "utf-8"));
			return "message";
		} catch (Exception e) {
			e.printStackTrace();
			try {
				request.setAttribute("error", URLEncoder.encode("图书更新失败!","utf-8"));
	...
	public String delete(@PathVariable String barcode, HttpServletRequest request) {
		try {
			bookService.deleteBook(barcode);

			request.setAttribute("message", java.net.URLEncoder.encode("图书删除成功!", "utf-8"));
			return "message";
		} catch (Exception e) {
			e.printStackTrace();
			try {
				request.setAttribute("error", URLEncoder.encode("图书删除失败!", "utf-8"));
			} catch (UnsupportedEncodingException e1) {
				e1.printStackTrace();
			}
			return "error";
	...
	public String OutToExcel(String barcode, @ModelAttribute BookType bookType, String bookName, String publishDate,
		...
		try {
			...
			for (int i = 0; i < bookList.size(); i++) {
				...
				dataset.add(new String[] { book.getBarcode(), book.getBookName(), book.getBookType().getBookTypeName(),
						book.getPrice() + "", book.getCount() + "", book.getPublish(), book.getPublishDate(),
						book.getPhotoBook() });
			}
			/*
			 * OutputStream out = null; try { out = new
			 * FileOutputStream("C://output.xls"); ex.exportExcel(title,headers,
			 * dataset, out); out.close(); } catch (Exception e) {
			 * e.printStackTrace(); }
			 */
